...
\begin{table}[bp]
	...
	\begin{tabular}{| c | c | c | c |}
		 ...
		 Rotational Velocity & \multicolumn{3}{c|}{2500 RPM (kept constant)} \\ \hline
...
Table 2 shows that in general, the optimizer kept the moments and torques experienced by the rotor below their maximum values. This was done in the code by finding the maximum values for the unoptimized rotor, then multiplying these by the safety factor, in this case 1.2, and turning them into global variables that could be referred to at each step. \newline
...
\begin{tabular}{| c | c | c | c |}
	 ...
	 \textbf{2 Blades} & 0.517 & $4.739 \times 10^{6}$ & $4.209 \times 10^{5}$ \\
	 \textbf{3 Blades} & 0.645 & $4.591 \times 10^{6}$ & $4.184 \times 10^{5}$ \\
	 \textbf{4 Blades} & 0.645 & $3.752 \times 10^{6}$ & $3.119 \times 10^{5}$ \\ \hline
\end{tabular} \break \newline

\raggedright
The optimizer will seek for a minimum value, but I am trying to maximize my objective function. I will use the reciprocal of the objective function in the optimization. In summary, the optimization will seek to do these things: \newline
...
This report optimized the twist and chord scaling of rotors with 2, 3, and 4 blades. Some conclusions can be drawn from these results, which are displayed in table 4. Increasing the blade count changed both the optimal chord scaling and twist angle. \newline
...
Figure 1 shows that the optimized rotors fulfilled the objective function better than the original rotor did. They increased the product of the effectiveness, $\eta$, and the thrust coefficient, $C_{T}$, at the advance ratio selected for the optimization, which was found at the 45 m/s free stream velocity, 20 m outer diameter, and 2500 RPM rotational velocity summarized in table 1. After the propellor was optimized at the advance ratio shown below, it was measured at a variety of different advance ratios. \newline

\begin{equation}
\begin{aligned}
	J = \frac{v_{a}}{nD} = \frac{v_{a}}{\frac{RPM}{60}D} \\
	J = \frac{45}{\frac{2500}{60} \times 20} = 0.054 \\
\end{aligned}
\end{equation} 
...
In conclusion, the optimal angle of rotation for a rotor to maximize both its thrust coefficient and efficiency is slightly negative. The optimal thickness is larger than the rotor thickness for propellors with only one or two blades, but it decreases until an optimal propellor is thinner than the default blade for higher blade counts.
