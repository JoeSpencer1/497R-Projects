...
The plots in Figure \ref{fig:1} and Figure \ref{fig:4} appear to have a similar upward slope, which is expected. As the wing tilts upwards with an increasing angle of attack \hyperref{aplha}{$\alpha$}, its \hyperlink{CL}{Lift Coefficient} increases from negative to positive for both airfoils. A notable difference, though, is that for the airfoils with their maximum camber further forward shown in Figure \ref{fig:5} had a much greater drag coefficient at lower angles than those in Figure \ref{fig:2}.
